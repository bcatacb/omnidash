...
export async function deleteMessage(conversationId: string, messageId: string): Promise<void> {
  await query('DELETE FROM tenant_main.messages WHERE conversation_id = $1 AND id = $2', [conversationId, messageId]);
}
...
export async function updateConversationLastMessage(conversationId: string): Promise<void> {
  ...
  if (rows.length > 0) {
    ...
    await query(
      `UPDATE tenant_main.conversations 
       SET last_message_preview = $1, last_message_at = $2, last_message_direction = $3 
       WHERE id = $4`,
      [r.body.slice(0, 80), r.sent_at, r.direction, conversationId]
    );
  } else {
    await query(
      `UPDATE tenant_main.conversations 
       SET last_message_preview = '', last_message_at = NULL, last_message_direction = NULL 
       WHERE id = $1`,
      [conversationId]
    );
  }
...
/** Finds any lead (including already-replied ones) for a given account + discord peer.
 *  Used for "campaign-associated reply" detection and Telegram notifications. */
export async function findLeadByDiscordUserId(accountId: string, discordUserId: string): Promise<LeadRow | null> {
  const rows = await query<any>(
    `SELECT * FROM tenant_main.leads
       WHERE assigned_account_id = $1 AND discord_user_id = $2
       ORDER BY COALESCE(dm_sent_at, created_at) DESC
       LIMIT 1`,
    [accountId, discordUserId],
  );
  return rows[0] ? rowToLead(rows[0]) : null;
...
export async function getAlreadyContactedDiscordUserIds(): Promise<Set<string>> {
  // Union both outreach (leads) and FR (fr_campaign_leads) so no user gets
  // re-contacted by a different account in any campaign type.
  const rows = await query<any>(
    `SELECT discord_user_id FROM tenant_main.leads WHERE status IN ('sent','replied')
     UNION
     SELECT discord_user_id FROM tenant_main.fr_campaign_leads WHERE status IN ('fr_sent','fr_accepted','dm_sent')`,
  );
...
export async function importFrLeads(campaignId: string, leads: Array<{ discord_user_id: string; display_name?: string; username?: string }>): Promise<{ inserted: number; skipped: number }> {
  if (!leads.length) return { inserted: 0, skipped: 0 };
  const contacted = await getAlreadyContactedDiscordUserIds();
  const fresh = leads.filter((l) => !contacted.has(l.discord_user_id));
...
export async function pushMembersToFrCampaign(opts: {
  ...
  const allMembers = await query<{ id: number; discord_user_id: string; username: string; global_name: string | null }>(
  ...
  const contacted = await getAlreadyContactedDiscordUserIds();
  const members = allMembers.filter((m) => !contacted.has(m.discord_user_id));

  const ids = allMembers.map((m) => Number(m.id));
  await query(
...
// ───── Saved Invite Code Groups ──────────────────────────────────────────────
// Persistent groups of invite codes, grouped by server, for reuse when
// starting new join campaigns.

export interface SavedInviteGroup {
  id: string;
  name: string;
  guild_id: string;
  guild_name: string | null;
...
export async function listSavedInviteGroups(): Promise<SavedInviteGroup[]> {
  // Ensure table exists (for simplicity; in prod use migration)
  await query(`
    CREATE TABLE IF NOT EXISTS tenant_main.saved_invite_groups (
  ...
  `);
  return await query<SavedInviteGroup>(
    `SELECT * FROM tenant_main.saved_invite_groups ORDER BY created_at DESC`,
  );
}

export async function createSavedInviteGroup(data: {
  name: string; guild_id: string; guild_name?: string | null; guild_icon?: string | null; codes: string[];
}): Promise<SavedInviteGroup> {
  const rows = await query<SavedInviteGroup>(
    `INSERT INTO tenant_main.saved_invite_groups (name, guild_id, guild_name, guild_icon, codes)
     VALUES ($1,$2,$3,$4,$5) RETURNING *`,
    [data.name, data.guild_id, data.guild_name ?? null, data.guild_icon ?? null, data.codes],
  );
  return rows[0]!;
}

export async function updateSavedInviteGroup(id: string, patch: Partial<Pick<SavedInviteGroup, 'name' | 'codes'>>): Promise<void> {
  const sets: string[] = []; const vals: any[] = [id];
  if (patch.name !== undefined) { vals.push(patch.name); sets.push(`name=$${vals.length}`); }
  if (patch.codes !== undefined) { vals.push(patch.codes); sets.push(`codes=$${vals.length}`); }
  if (!sets.length) return;
  await query(`UPDATE tenant_main.saved_invite_groups SET ${sets.join(',')} WHERE id=$1`, vals);
}

export async function deleteSavedInviteGroup(id: string): Promise<void> {
  await query(`DELETE FROM tenant_main.saved_invite_groups WHERE id=$1`, [id]);
}
...
function generateJoinSchedule(count: number, joinsPerDay: number, firstMs: number): Date[] {
  ...
  while (scheduled < count) {
    ...
    for (let i = 0; i < todayCount; i++) {
      if (times.length === 0) {
        // First account in the campaign/batch: start immediately (no 5min+ delay).
        // User request: don't schedule the first one; if only one account, start right away.
        const now = Date.now();
        const jitter = Math.floor(Math.random() * 45000); // 0-45s
        times.push(new Date(now + jitter));
        lastOff = 0;
        continue;
      }
