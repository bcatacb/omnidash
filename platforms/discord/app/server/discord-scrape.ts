...
import { discordHeaders as buildDiscordHeaders, tlsFetch, forceRefreshDiscordBuildNumber } from "./discord-http";
import { browserFetch, browserFetchEnabled } from "./discord-browser";
...
export interface JoinInviteResult {
  ...
  captcha?: { sitekey: string; rqdata: string; rqtoken: string; service: string; sessionId?: string };
}
...
export async function joinByInvite(
  ...
  try {
    ...
    if (captchaSessionId) body.captcha_session_id = captchaSessionId;

    let r: any = null;
    let bodyText = '';
    const useBrowser = !!accountId && browserFetchEnabled();
    let postAttempt = 0;
    let lastParsed: any = null;
    while (postAttempt < 2) {
      postAttempt++;
      if (postAttempt > 1) {
        forceRefreshDiscordBuildNumber();
        console.log(`[join-invite] retrying POST with fresh build number after update message`);
      }
      const headers = await buildDiscordHeaders(token, true);
      if (useBrowser) {
        r = await browserFetch(accountId, token, url, {
          method: "POST",
          headers,
      ...
      if (r.ok) break;
      try { lastParsed = JSON.parse(bodyText); } catch { lastParsed = null; }
      const updateMsg = Array.isArray(lastParsed?.captcha_key) && lastParsed.captcha_key[0];
      if (updateMsg && /update your app/i.test(updateMsg) && postAttempt < 2) {
        console.warn(`[join-invite] Discord client update message (retrying with fresh build): ${updateMsg}`);
        continue;
      }
      break;
    }
    if (!r.ok) {
      if (lastParsed?.captcha_sitekey) {
        console.warn(`[join-invite] captcha challenge returned (post-solve or first) status=${r.status} body=${bodyText.slice(0, 700)}`);
        return {
          ok: false,
          httpStatus: r.status,
          captcha: {
            sitekey: String(lastParsed.captcha_sitekey),
            rqdata: String(lastParsed.captcha_rqdata || ''),
            rqtoken: String(lastParsed.captcha_rqtoken || ''),
            service: String(lastParsed.captcha_service || 'hcaptcha'),
            sessionId: String(lastParsed.captcha_session_id || ''),
          },
          error: 'captcha required',
        };
      }
      console.warn(`[join-invite] POST ${url} failed status=${r.status} body=${bodyText.slice(0, 500)}`);
      let msg: string;
      if (lastParsed) {
        msg = lastParsed.message || (lastParsed.errors ? JSON.stringify(lastParsed.errors) : '') || bodyText.slice(0, 200);
      } else {
        msg = bodyText.slice(0, 200);
      }
      if (!msg || msg === 'Unknown Message') {
        msg = `Discord error (status ${r.status}): ${bodyText.slice(0, 300)}`;
      }
      return { ok: false, httpStatus: r.status, error: msg };
    }
    const j = (bodyText ? JSON.parse(bodyText) : {}) as any;
    const guildId = j?.guild?.id ? String(j.guild.id) : undefined;
    const guildName = j?.guild?.name ? String(j.guild.name) : undefined;
    const channelId = j?.channel?.id ? String(j.channel.id) : undefined;

    // Many servers have Membership Screening ("acknowledge rules") enabled.
    // The invite POST succeeds but the member is left in pending state.
    // A follow-up PATCH to the member endpoint usually clears the requirement
    // so the account can immediately chat without seeing the rules modal.
    if (guildId) {
      const patchUrl = `https://discord.com/api/v9/guilds/${encodeURIComponent(guildId)}/members/@me`;
      const patchBody = JSON.stringify({});
      const patchHeaders = await buildDiscordHeaders(token, true);
      if (useBrowser) {
        browserFetch(accountId!, token, patchUrl, {
          method: "PATCH",
    ...
    return { ok: true, guildId, guildName, channelId };
  } catch (err: any) {
