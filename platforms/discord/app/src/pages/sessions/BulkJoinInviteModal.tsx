...
export default function BulkJoinInviteModal({
  ...
  const [activeCaptcha, setActiveCaptcha] = useState<{ rowIdx: number; sitekey: string; rqdata: string; rqtoken: string; sessionId?: string } | null>(null)
  ...
  const tryJoin = async (rowIdx: number, captchaKey?: string, captchaRqtoken?: string, captchaSessionId?: string) => {
    ...
    try {
      ...
      if (captchaSessionId) body.captchaSessionId = captchaSessionId
      ...
      if (r.status === 400 && captcha?.sitekey) {
        setRows((prev) => prev.map((rr, i) => (i === rowIdx ? {
          ...
          captcha: {
            ...
            sessionId: String(captcha.sessionId || ""),
          },
        } : rr)))
        return
      }
  ...
  const openSolve = (rowIdx: number) => {
    ...
    setActiveCaptcha({
      ...
      sessionId: row.captcha.sessionId,
    })
          ...
          <div className="mt-3 rounded-md border border-amber-500/30 bg-amber-500/10 p-3 text-[12px]">
            ...
            <HCaptcha
              ...
              onVerify={(token) => {
                ...
                const sessionId = activeCaptcha.sessionId
                setActiveCaptcha(null)
                void tryJoin(rowIdx, token, rqtoken, sessionId)
              }}
              onError={() => {
