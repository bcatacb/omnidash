...
export default function WarmupMonitor() {
  ...
  const addAccount = async () => {
    ...
    if (r && !r.ok) {
      ...
      await notify({ title: "Failed to add account", description: body.error || "Unknown error", variant: "error" })
      return
    }
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <section className="rounded-card border border-bg-tertiary bg-bg-secondary p-3">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    await notify({
                      ...
                      description: `Paired ${r.created ?? 0} new pairs (${r.skippedSameProxy ?? 0} skipped same-proxy). ${unpaired} accounts now have partners.`,
                    })
                  }}
        ...
        <div className="max-h-[320px] overflow-y-auto rounded border border-bg-tertiary">
        <table className="w-full text-[12px]">
          <thead className="sticky top-0 bg-bg-secondary text-text-muted z-10">
            <tr><th className="text-left px-2 py-1">Account</th><th className="text-center">Sent</th><th className="text-center">Pairs</th><th className="text-center">Last sent</th><th className="text-center">Next send</th><th className="text-center">Status</th><th></th></tr>
          </thead>
          <tbody>
            {accts.map((a) => {
