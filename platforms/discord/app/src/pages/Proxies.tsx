...
export default function Proxies() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [pResp, aResp] = await Promise.all([
        fetch("/api/proxies", { cache: "no-cache" }),
  ...
  const deleteProxy = async (id: string) => {
    if (!await confirm({ title: "Delete this proxy?", description: "Assigned accounts will lose their routing.", variant: "danger" })) return
