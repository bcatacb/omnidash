...
import { listGuilds as listAccountGuilds, scrapeGuildMembers, scrapeGuildMembersSmart, joinByInvite } from './discord-scrape';
...
app.post('/api/unibox/conversations/:id/send', async (req, res) => {
  ...
  if (convId.startsWith('live_')) {
    ...
    // Forward recipient identity so the browser wrapper's Tier 3 (Cmd+K quick
    // switcher) and Tier 4 (server member-list drive) have a name + id to work
    // with when the direct URL doesn't render the editor.
...
app.post('/api/accounts/:id/join-invite', async (req, res) => {
  ...
  const captchaKey = typeof req.body?.captchaKey === 'string' ? req.body.captchaKey : undefined;
  const captchaRqtoken = typeof req.body?.captchaRqtoken === 'string' ? req.body.captchaRqtoken : undefined;
  const result = await joinByInvite(token, invite, captchaKey, captchaRqtoken);
  if (result.captcha) {
...
app.post('/api/groups/:id/bulk-join', async (req, res) => {
  ...
  for (const m of group.members) {
    ...
    const r = await joinByInvite(token, invite);
    if (r.captcha) {
