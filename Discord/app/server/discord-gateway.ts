...
function handleDispatch(conn: GatewayConn, type: string, d: any) {
  ...
  switch (type) {
    ...
    case "MESSAGE_CREATE": {
      ...
      const conv = state.conversations.find((c) => c.id === convId && c.accountId === conn.accountId);
      if (!conv) {
        // First message from a DM we haven't imported yet. Let the next poll pick it up.
        return;
      }
    ...
    case "CHANNEL_CREATE": {
      ...
      if (channelType !== 1 && channelType !== 3) break;
      ...
      const peerName = channelType === 3
        ? (d?.name || (d?.recipients?.map((r: any) => r.global_name || r.username).join(", ") ?? "Group DM"))
    ...
    case "CHANNEL_DELETE": {
      ...
      // a guild channel they could see gets deleted). For us, only DM channels
      // matter — type 1 = DM, type 3 = group DM. Anything else, ignore.
      const channelType = Number(d?.type);
      if (channelType !== 1 && channelType !== 3) break;
      const channelId = String(d?.id || "");
      if (!channelId) break;
      const convId = `live_${channelId}`;
      // Only remove the conversation that belongs to THIS account. Group DMs
      // can be shared across our accounts — closing one account's view of the
      // group shouldn't drop the other account's row.
