...
async function twoCaptchaSolveWithRetry(opts: SolveTokenOpts, apiKey: string, proxyUrl?: string): Promise<SolveTokenResult> {
  // Always use form API (enterprise=1 + data=rqdata for Enterprise challenges).
  // The JSON API's gRecaptchaResponse is rejected by Discord with "invalid-response"
  // for Enterprise hCaptcha. The form API token format is what Discord expects.
  const first = await twoCaptchaSolve(opts, apiKey, 1, proxyUrl, false);
  if (first.ok && first.token) {
    return { ok: true, token: first.token, rqtoken: opts.rqtoken, solverUsed: "twocaptcha", costCents: first.costCents || 0, attempts: 1 };
  }
  if (first.unsolvable) {
    // Switch to JSON Enterprise API on retry when rqdata is present (Discord join captchas).
    // Form API workers often fail on Enterprise challenges; JSON API routes to a better pool.
    const useJsonApiOnRetry = !!opts.rqdata;
    console.log(`[captcha] 2captcha UNSOLVABLE on attempt 1 — retrying with ${useJsonApiOnRetry ? 'json-enterprise' : 'form'} api`);
    const second = await twoCaptchaSolve(opts, apiKey, 2, proxyUrl, useJsonApiOnRetry);
    if (second.ok && second.token) {
      return { ok: true, token: second.token, rqtoken: opts.rqtoken, solverUsed: "twocaptcha", costCents: second.costCents || 0, attempts: 2 };
    }
...
async function twoCaptchaSolve(
  ...
  if (isEnterprise) {
    ...
    // Use HCaptchaTask (with proxy) when a relay is available so the solving IP
    // matches the join request IP. Fall back to Proxyless only when no proxy is set.
    ...
    // Report bad to free the slot — do not await, best-effort
    ...
    let solveRelay: ProxyRelay | null = null;
    if (proxyUrl) {
      // Spin up a temporary per-solve relay port on the VPS so 2captcha
      // tunnels through the account's residential proxy. No credentials
      // needed — the port itself identifies which upstream to use.
      solveRelay = await startSolveRelay(proxyUrl).catch(() => null);
      if (solveRelay) {
